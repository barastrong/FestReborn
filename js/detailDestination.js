document.addEventListener('DOMContentLoaded', () => {
    ...
    if (data.ticket === null || data.ticket === 0) {
        ...
        const formattedTicket = new Intl.NumberFormat('id-ID').format(data.ticket);
        ticketPriceEl.innerHTML = `Rp ${formattedTicket} <span class="text-base font-medium text-slate-500">/ orang</span>`;
    }

    const phoneButton = document.getElementById('phone-button');
    if(data.phone && data.phone.replace('tel:', '').trim() !== '-') { 
        ...
        descriptionContainer.innerHTML = data.contentBlocks.map(block => {
            ...
            if (block.type === 'image' && block.images && block.images.length > 0) {
                if (block.images.length === 1) {
                    return `<div class="rounded-xl overflow-hidden shadow-lg my-6"><img src="${block.images[0]}" class="w-full h-auto object-cover" alt="Ilustrasi deskripsi"></div>`;
                } else {
                    const imageElements = block.images.map(img => `<img src="${img}" class="w-full h-64 object-cover rounded-xl shadow-sm" alt="Ilustrasi deskripsi">`).join('');
                    return `<div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">${imageElements}</div>`;
                }
            }
            return '';
        }).join('');
    ...
    thumbnailGrid.innerHTML = data.images.map((imgSrc, index) => `<img src="${imgSrc}" data-index="${index}" class="thumbnail-item w-full h-24 object-cover rounded-md cursor-pointer border-2 border-transparent hover:border-primary transition-all opacity-70 ${index === 0 ? 'opacity-100 border-primary' : ''}" alt="Thumbnail ${index + 1}">`).join('');

    const thumbnails = document.querySelectorAll('.thumbnail-item');
    function setActiveThumbnail(index) {
        thumbnails.forEach(thumb => {
            thumb.classList.remove('opacity-100', 'border-primary');
        ...
        if (activeThumb) {
            activeThumb.classList.remove('opacity-70', 'border-transparent');
            activeThumb.classList.add('opacity-100', 'border-primary');
        }
    }
    thumbnails.forEach(thumb => {
    ...
    const shareText = `Yuk, Kunjungi ${data.name} di Sidoarjo! Info lengkap destinasi ada di sini: ${pageUrl}`;
