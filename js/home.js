document.addEventListener('DOMContentLoaded', () => {
  const topDestinations = [
    ...
    { name: "Candi Pari", image: "./assets/candi-pari.jpg" },
    { name: "Pulau Sarinah (Pesisir Timur)", image: "./assets/pulau-sarinah.jpg" },
    ...
    { name: "Kebun Cokelat Balongbendo", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSgN_BpjYi5bFfahDiZhZtofIqosTB1ljq-Uw&s" },
    ...
    { name: "Pasar Malam GOR Sidoarjo", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRQu1kLikb8Ni95b4tgx_fOyRlvE6H-IDuHmA&s" },
    { name: "Sentra Kuliner G-Walk", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRfvT5JHs45DCEkRq9tmu0hU2lV5m0uFMUiMg&s" }
  ];
  ...
  if (destinationGrid) {
    destinationGrid.innerHTML = topDestinations.map(dest => {
      ...
      return `
        <a href="detailDestination.html?id=${id}" class="flex-shrink-0 w-64 md:w-72">
          <div class="relative rounded-xl overflow-hidden aspect-[3/4] shadow-lg">
            ...
            <div class="absolute bottom-0 left-0 p-4 md:p-5 text-white w-full">
              ...
              <p class="text-sm opacity-80 flex items-center gap-2">Kunjungi <i class="fa-solid fa-arrow-right text-xs"></i></p>
            </div>
          </div>
        </a>`;
    }).join('');
    ...
    packageGrid.innerHTML = tourPackages.map(pkg => {
      const features = pkg.features.map(f => `
        <li class="flex items-center">
          <i class="fa-solid ${f.included ? 'fa-check text-primary' : 'fa-xmark text-slate-400'} mr-3"></i> ${f.text}
        </li>`).join('');
      return `
        <div class="bg-white rounded-2xl shadow-lg p-6 md:p-8 border-t-4 border-primary-dark h-full flex flex-col fade-in-up">
          ...
          <button data-package-id="${pkg.id}" class="w-full text-center mt-10 py-3 px-6 rounded-lg font-semibold border-2 border-primary-dark text-primary-dark transition">Pilih Paket Ini</button>
        </div>`;
    }).join('');
  }
    ...
    testimonialGrid.innerHTML = testimonials.map(t => `
      <div class="relative bg-white p-8 rounded-2xl shadow-xl fade-in-up">
        <div class="flex items-center gap-4 mb-6">
          ...
          <div><h4 class="font-bold text-slate-800 text-lg">${t.name}</h4><p class="text-sm text-slate-500">${t.origin}</p></div>
        </div>
  ...
  const openModal = id => {
    ...
    featuresContainer.innerHTML = data.features.map(f => `
      <li class="flex items-center text-sm ${f.included ? 'text-slate-700' : 'text-slate-400 line-through'}">
        <i class="fa-solid ${f.included ? 'fa-check-circle text-primary' : 'fa-times-circle'} mr-3"></i>${f.text}
      </li>`).join('');
    ...
    itineraryContainer.innerHTML = data.itinerary.map((item, i) => `
      <div class="relative pl-10 border-l-2 border-slate-200 ${i === data.itinerary.length - 1 ? 'last:border-l-0 last:pb-0' : 'pb-8'}">
        <div class="absolute -left-3 top-0 w-6 h-6 rounded-full bg-primary-dark border-4 border-white flex items-center justify-center">
        ...
        <div class="ml-4">
          ...
          <div class="flex justify-between items-center">
            <h4 class="text-lg font-bold text-slate-800">${item.destination}</h4>
              ${item.price ? `<span class="text-sm font-semibold text-primary-dark">${item.price} /orang</span>` : ''}
          </div>
